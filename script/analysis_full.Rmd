...
### Plot 1: Male responses differ before/after exhibition.
...
Q_df <- data.frame(Qnumber = paste0("Q", 1:5),
...
text_data <- data.frame(x = c(0.1),
                        y = c(20),
                        label = c(glue::glue("<span style='color:{sig_colour}'>**Personal responsibility (Q3)**</span><br>showed a significant difference", sig_colour = sig_colour)))
...
plot_data %>% 
  {ggplot() +
      ...
      geom_line(data = filter(., sig),
                aes(x = Treatment, y = mean*100, group = QuestionNumber), size = 1, colour = sig_colour) +
      ...
      labs(title = glue::glue("<span style = 'font-size:{title_size}pt; color:{emph_colour}'>Men were</span><span style = 'font-size:{bigtitle_size}pt; color: {sig_colour}'> **more likely** </span><span style = 'font-size:{title_size}pt; color: grey25'>to strongly agree with statements<br>after seeing the exhibition</span>", title_size = title_size,
           ...
           caption = glue::glue("<span style='font-size:{caption_size}pt; color:{nonemph_colour}'>Data: Survey data from In Your Face exhibition in Berlin (N = 35) and Kiel (N = 8)</span>", caption_size = caption_size, nonemph_colour = nonemph_colour)) +
...
ggplot2::ggsave(file = here::here("./plot/Male_effect.png"), height = 5, width = 7, dpi = 600)

```

#### Plot 1b: Combined plot
...
#Create plot data for males only
plot_data <- raw_data %>% 
  group_by(Gender, Treatment, QuestionNumber) %>% 
  summarise(binom::binom.wilson(x = sum(value), n = n()), .groups = "drop") %>% 
  dplyr::mutate(Treatment = as.numeric(Treatment),
                #Create a separate category for Q3 (personal responsibility) which was significant
                sig = Gender == "Male" & QuestionNumber == "Q3")

#This data frame is used to create in graph highlight text (i.e. to emphaises/explain key results)
text_data <- data.frame(x = c(0.1),
                        y = c(20),
                        Gender = c("Male"),
                        label = c(glue::glue("<span style='color:{sig_colour}; font-weight: bold'>**Personal responsibility (Q3)**</span><br>showed a significant difference in men", sig_colour = sig_colour)))

#Create plot
plot_data %>% 
  {ggplot() +
      #Fit slopes for every question. Fit separately for non-significant and significant data so that significant data is at front
      geom_line(data = filter(., !sig), aes(x = Treatment, y = mean*100, group = QuestionNumber, colour = Gender), size = 1, colour = nonemph_colour) +
      geom_line(data = filter(., sig), aes(x = Treatment, y = mean*100, group = QuestionNumber), size = 1, colour = sig_colour) +
      #Fit before/after points for every question. Fit separately so that significant data is at front
      geom_point(data = filter(., !sig), aes(x = Treatment, y = mean*100, group = QuestionNumber), size = 3.25, colour = nonemph_colour) +
      geom_point(data = filter(., sig), aes(x = Treatment, y = mean*100, group = QuestionNumber), size = 3.25, colour = sig_colour) +
      #Add labels to show which line is associated with which question
      ggtext::geom_richtext(data = filter(., Treatment == 0),
                            aes(x = Treatment - 0.55,
                                y = mean*100,
                                label = rep(Q_df$Short_wording, times = 2),
                                colour = sig),
                            #Left justify to create clean lines
                            hjust = 0,
                            #Remove box around text
                            ...
                            lineheight = 0.2) +
      #Add percentage values before exhibition
      ggtext::geom_richtext(data = filter(., Treatment == 0), aes(x = Treatment - 0.07,
                                                                  y = mean*100,
                                                                  label = glue::glue("<span style = 'font-size:{label_size}pt;'>{number}%</span>",
                                                                                     number = round(mean*100),
                                                                                     label_size = label_size),
                                                                  colour = sig),
                            fill = NA, label.color = NA) +
      #Add percentage values after exhibition. Remove Q4 for males because value is identical to Q3
      ggtext::geom_richtext(data = filter(., Treatment == 1 & !(QuestionNumber == "Q4" & Gender == "Male")), aes(x = Treatment + 0.07,
                                                                                                                 y = mean*100,
                                                                                                                 label = glue::glue("<span style = 'font-size:{label_size}pt;'>{number}%</span>",
                                                                                                                                    number = round(mean*100),
                                                                                                                                    label_size = label_size),
      ...
      ggtext::geom_richtext(data = text_data,
                            ...
                            hjust = 0) +
      #Add labels to specify before/after
      ggtext::geom_richtext(data = data.frame(x = c(0, 1, 0, 1),
                                              y = c(95, 95, 95, 95),
                            ...
                            fill = NA, label.color = NA, colour = emph_colour) +
      #Add title, subtitle, and caption
      labs(title = glue::glue("<span style = 'font-size:{title_size}pt; color:{emph_colour}'>Men were</span><span style = 'font-size:{bigtitle_size}pt; font-weight: bold; color: #33658a'> **more likely** </span><span style = 'font-size:{title_size}pt; color: grey25'>to strongly agree with statements<br>after seeing the exhibition</span>", title_size = title_size, bigtitle_size = title_size + 3, emph_colour = emph_colour),
           subtitle = glue::glue("<span style='font-size:{subtitle_size}pt; color:{nonemph_colour}'>Percentage of respondants that answer 'strongly agree'</span>", subtitle_size = subtitle_size, nonemph_colour = nonemph_colour),
           caption = glue::glue("<span style='font-size:{caption_size}pt; color:{nonemph_colour}'>Data: Survey data from In Your Face exhibition in Berlin (N = 35) and Kiel (N = 8)</span>", caption_size = caption_size, nonemph_colour = nonemph_colour)) +
      #Specify the colour of significant text/lines etc
      scale_colour_manual(values = c(nonemph_colour, sig_colour)) +
      #Settings for x axis (many of these are redundant given theme but keep them incase we shift back)
...
ggplot2::ggsave(file = here::here("./plot/Gender_effects.png"), height = 7, width = 14, dpi = 600)

```

### Plot 1c: gt table to link 'Q1' to actual question wording

Create data frame of questions
...
(Q_df %>% 
    ...
    tab_header(title = md("Survey questions")) %>% 
    #Add a data source footnote
    tab_source_note(source_note = "Data: Ocean.Now! 'In Your Face' survey") %>% 
